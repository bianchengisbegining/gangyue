...
dbengine = create_engine('mysql+pymysql://root:'+databasePWD+'@localhost/gangyue')
...
def sign_in():
	if request.method == 'POST':
		pymysql_db = pymysql.connect(host=sqlconn['host'],user=sqlconn['user'],password=sqlconn['password'],database=sqlconn['database'])	 
...
def main(page=1):
	...
	paginate = Post.query.order_by(Post.added_time.desc()).paginate(page,
														  per_page=10,
														  error_out=False)
	data = {
		'userName': userName,
...
def post_new():
	if request.method == 'GET':
		...
		return render_template('post_new.html',userName=userName,userPsd=userPsd )
	if request.method == 'POST':
		userName = request.cookies.get('userName')
		userPsd = request.cookies.get('userPsd')
		...
		try:
			...
			post = Post(id=id,category=category,title=title,added_time=added_time,user_id=userId)
